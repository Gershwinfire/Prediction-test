...
const predict = async () => {
    try {
        ...
        const response = await axios.get(`https://finnhub.io/api/v1//crypto/candle?symbol=BINANCE:BTCUSDT&resolution=D&count=1&token=cf2c4u2ad3idqn4q5ba0cf2c4u2ad3idqn4q5bag`);
        ...
        if (rsi > 70 && slopeRsi > 0 && slopeEma > 0 && ema > data.c[data.c.length - 1]) {
            console.log("Next 30min wick is likely to be red");
        } else if (rsi < 30 && slopeRsi < 0 && slopeEma < 0 && ema < data.c[data.c.length - 1]) {
